...
-- Nome do aluno:Victor Bueno Diniz
...
SELECT 
    IP.NumeroPedido AS NumeroPedido, 
    COUNT(DISTINCT IP.CodigoProduto) AS QuantidadeProdutosDiferentes,
    SUM(IP.Quantidade) AS TotalItens
FROM ITEM_DE_PEDIDO IP
GROUP BY IP.NumeroPedido;
...
SELECT 
    ...
    Pd.PrazoEntrega AS PrazoEntrega, 
    V.NomeVendedor AS NomeVendedor
FROM CLIENTES C
JOIN PEDIDOS Pd ON C.CodigoCliente = Pd.CodigoCliente
JOIN ITEM_DE_PEDIDO IP ON Pd.NumeroPedido = IP.NumeroPedido
JOIN PRODUTOS P ON IP.CodigoProduto = P.CodigoProduto
JOIN VENDEDORES V ON Pd.CodigoVendedor = V.CodigoVendedor
ORDER BY 
    C.NomeCliente, 
    P.DescricaoProduto, 
    IP.Quantidade, 
